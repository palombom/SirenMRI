...
for i in range(int(n_walkers/b_size)):
    for j in range(coord):
        ...
        if 'train' in args.operation:
            ...
            # Save best model
            torch.save(trainer.best_model, compression_folder + f'/best_model_batch_{i}_{j}.pt')

            # Update current model to be best model
            func_rep.load_state_dict(trainer.best_model)
        else:
            print("")
            print(f'Decompressing batch: {i} - coord {j}')

            init_batch = init_pos[i * b_size:(i + 1) * b_size, :]
            coordinates = torch.Tensor(init_batch)

            func_rep.load_state_dict(torch.load(compression_folder + f'/best_model_batch_{i}_{j}.pt'))

        if 'decompress' in args.operation:
            # Save full precision image reconstruction
            with torch.no_grad():
                PredParams = func_rep(coordinates)
            PredParams = scaler[j][i].inverse_transform(PredParams.cpu().numpy())
            trj_decompressed[i * b_size:(i + 1) * b_size, :, j] = np.reshape(PredParams,
                                                                             (b_size, trj_decompressed.shape[1]))
